...
bool HelloWorld::init()
{
    ...
    auto bottomSprite = Sprite::create("bottom.jpg");
    ...
    replayItem->setScale(scaleFactor);
    auto btnReplay = Menu::create(replayItem, NULL);
    ...
    schedule(schedule_selector(HelloWorld::updateTimer), 0.2f);
...
void HelloWorld::drawMap()
{
    ...
    if (height > width) {
        switch (height) {
            case 3:
                imageSizeHeight = imageSizeWidth = 250 * 0.5f;
                break;
            case 4:
                imageSizeHeight = imageSizeWidth = 250 * 0.4f;
                break;
            case 5:
                imageSizeHeight = imageSizeWidth = 250 * 0.3f;
                break;
            default:
                break;
        }
    ...
    if (GameHUD::shareInstance()->getTime() <= 0) {
        ...
        CCARRAY_FOREACH(pics, child){
            ...
            if (sprite->getTag() == 2) {
                ...
                sprite->runAction(RepeatForever::create(Sequence::create(scaleUpAction, scaleDownAction, NULL)));
                break;
            }
        }
    }
        ...
        CCARRAY_FOREACH(pics, child){
            ...
            if (sprite->getTag() == 2) {
                ...
                if (spRect.containsPoint(location)) {
                    isTouchRight = true;
                    log("right!");
                    if (GameHUD::shareInstance()->getTime() > 100 * 2/3) {
                        gameScore += 2;
                    }else
                        if (GameHUD::shareInstance()->getTime() > 100 /3) {
                            gameScore += 1;
                        }
